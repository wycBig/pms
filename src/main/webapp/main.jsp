...
<div id="showdiv" style="width: 60%; margin: 0 auto; height:500px; border: 1px solid #999; display: none; position: absolute; top: 20%; left: 20%; z-index: 3; background: #fff">
	...
	<!-- 内容 -->
	<div id="content" style="text-indent: 50px; height: 4rem; font-size: 30px; padding: 0.5rem; line-height: 4rem; ">
    ...
    <table width="98%" align="center" border="0" cellpadding="3"
		...
		<tr bgcolor="#FFFFFF">
			<td>
				<ul class="notice-list" id="notice-list-four">
...
<script type="text/javascript">
    $(function () {
        $.ajax({
            type:"get",
            url:"${pageContext.request.contextPath}/notice/getThree",
            success:function (msg) {
                $(msg.map.list).each(function (index,item) {
                    var li = "<li class='ue-clear'>\n" +
                        "                    <span>12-15</span>&nbsp;&nbsp;&nbsp;<a onclick='showWindow("+item.nid+")' href='javascript:void(0)'class='notice-title'>"+item.ntitle+"</a>\n" +
                        "                </li>\n" +
                        "                <p>"
    ...
    $(function () {
        $.ajax({
            type:"get",
            url:"${pageContext.request.contextPath}/tz/getFour",
            success:function (msg) {
                $(msg).each(function (index,item) {
                    var li = "<li class='ue-clear'>\n" +
                        "\t\t\t\t\t发布时间："+moment(item.createtime).format("YYYY-MM-DD")+"\n" +
                        "\t\t\t\t\t<a href='role.jsp'class='notice-title'>"+item.forumtitle+"</a>\t\t\t\t\t\t\n" +
                        "\t\t\t\t\t</li>\n" +
                        "\t\t\t\t\t<p>";
                    $("#notice-list-four").append(li);
                })
            }
        })
    })
	function showWindow(obj) {
	    $.ajax({
			type:"get",
            url:"${pageContext.request.contextPath}/notice/getOne/"+obj,
            success:function (msg) {
